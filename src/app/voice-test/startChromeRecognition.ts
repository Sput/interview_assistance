// Lightweight, browser-only wrapper around SpeechRecognition with auto-restart
...
export function startChromeRecognition(
  ...
  const start = () => {
    ...
    rec.continuous = true; // best-effort in Chrome
    ...
    rec.onerror = () => {
      // Let onend handle the restart/backoff
    };
    ...
    rec.onend = () => {
      ...
      backoff = backoff ? Math.min(backoff * 2, 2000) : 100; // 100→200→400→...≤2000
